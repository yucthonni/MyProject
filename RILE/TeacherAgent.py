...
class TeacherAgent():
    def __init__(self, state_dim, action_dim, env, expert_trajectory):
        ...
        self.model = PPO(state_dim+action_dim, action_dim,
                         hidden_dim, self.replay_buffer)
        self.discriminator = Discriminator(
            state_dim+action_dim, hidden_dim, 128, self.trajectory_buffer, expert_trajectory)

    def ComputeReward(self):
        pb = tqdm(range(min(self.trajectory_buffer.index,
                  self.trajectory_buffer.buffer_size)))
        for i in pb:
            sa_pair = torch.cat(
                (self.trajectory_buffer.state[i], self.trajectory_buffer.action[i]), -1)
    ...
    def trainPPO(self, total_timestep: int, batch_size: int,progress_bar=False):
        if progress_bar:
    ...
    def train(self, total_timestep: int, PPO_timestep: int, D_timestep: int):
        ...
        for i in pb:
            ...
            print('Training PPO...')
            self.trainPPO(PPO_timestep, 1024)
            pb.update()

...
class SpecReplayBuffer:
    ...
    def from_after_reward(self,index:list):
        length=len(self.state[0])
        reward=[torch.FloatTensor(np.array(self.get_state_padding(self.reward[int(i/length)],i%length,10,True)[1])).view(-1) for i in index]
        return torch.stack(reward,dim=0).detach()

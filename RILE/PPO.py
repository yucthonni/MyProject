...
class PPO:
    ...
    def update(self,batch_size):
        ...
        old_states = torch.stack([self.buffer.state[i] for i in index],dim=0).detach().to(device)
        old_actions = torch.stack([self.buffer.action[i] for i in index],dim=0).detach().to(device)
        old_logprobs = torch.stack([self.buffer.logprobs[i] for i in index],dim=0).detach().to(device)
        old_state_values = torch.stack([self.buffer.state_value[i] for i in index],dim=0).detach().to(device)
